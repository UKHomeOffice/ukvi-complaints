...
function getWeeklyWindowUTC(anchorDate = new Date(), targetWeekday = 5) {
  const periodReferenceDate = new Date(anchorDate);

  // 0=Sun ... 6=Sat; default targetWeekday=5 (Friday)
  const daysBackToTarget = ((periodReferenceDate.getUTCDay() - targetWeekday + 7) % 7) || 7;

  // Compute the most recent occurrence of the target weekday at/before the anchor (UTC)
  const lastTargetWeekdate = subtractFromDate(periodReferenceDate, daysBackToTarget, 'days');
