 ...
 *
 * @param {number} hour - The hour to use (must be a number between 0 and 23)
...
function getUTCTime(hour, date = new Date()) {
  ...
  if (!(date instanceof Date) || isNaN(date.getTime())) {
    throw new Error('Invalid date object - a valid Date is required');
  }
...
/**
 * Computes the previous complete weekly window in UTC ending on a configurable weekday.
 *
 * Behavior:
 * - Anchors on the most recent `targetWeekday` at or before `anchorDate` (UTC).
 * - The window starts 6 days before that target weekday at 00:00:00 UTC.
 * - The window ends on the target weekday at 23:59:59 UTC.
 *
 * Defaults:
 * - `targetWeekday = 5` (Friday), producing a window from Saturday 00:00:00 to Friday 23:59:59.
 * - `anchorDate = new Date()` (current UTC time).
 *
 * @param {Date} [anchorDate=new Date()] - Anchor date (UTC) used to find the most recent target weekday.
 * @param {number} [targetWeekday=5] - Target weekday (0=Sun ... 6=Sat) on which the window ends.
 * @returns {{ start: Date, end: Date }} UTC Date objects for window start and end.
 */
function getWeeklyWindowUTC(anchorDate = new Date(), targetWeekday = 5) {
  if (!(anchorDate instanceof Date) || isNaN(anchorDate.getTime())) {
    throw new Error('Invalid anchorDate - must be a valid Date object');
  }
  if (typeof targetWeekday !== 'number' || isNaN(targetWeekday) || targetWeekday < 0 || targetWeekday > 6) {
    throw new Error('Invalid targetWeekday - must be a number between 0 (Sunday) and 6 (Saturday)');
  }

  // 0=Sun ... 6=Sat; default targetWeekday=5 (Friday)
  const daysBackToTarget = ((anchorDate.getUTCDay() - targetWeekday + 7) % 7) || 7;

  // Compute the most recent occurrence of the target weekday at/before the anchor (UTC)
  const lastTargetWeekdate = subtractFromDate(anchorDate, daysBackToTarget, 'days');

  // Start of the window: 6 days before the target weekday (e.g., Saturday when target is Friday)
  const startOfWindowDate = subtractFromDate(lastTargetWeekdate, 6, 'days');

  // Window start at 00:00:00 UTC on the startOfWindowDate (6 days before targetWeekday)
  const start = getUTCTime(0, startOfWindowDate);

  // Window end at 23:59:59 UTC on the targetWeekday: take 00:00:00 on the day after targetWeekday and subtract 1 second
  const midnightAfterTarget = getUTCTime(0, subtractFromDate(lastTargetWeekdate, -1, 'day'));
  const end = subtractFromDate(midnightAfterTarget, 1, 'second');
