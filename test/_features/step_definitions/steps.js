...
Then('I select {string} link', async function (name) {
  await this.page.click(`input[value="${name}"]`);
}.bind(World));
...
Then('I fill {string} with {string} option', async function (field, value) {
  //await this.page.selectOption(`select[id="${field}"]`, `option[value="${value}"]`);
  await this.page.selectOption(`select[id="${field}"]`, value);
}.bind(World));

Then('I select option {string}', async function (name) {
  await this.page.click(`option[value="${name}"]`);
}.bind(World));
...
Then('I should be on the {string} page', async function (uri, heading) {
